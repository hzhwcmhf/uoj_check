	...
	function newSubmissionForm($form_name, $requirement, $zip_file_name_gen, $handle) {
		...
		foreach ($requirement as $req) {
			if ($req['type'] == "source code") {
				...
				$form->addSourceCodeInput("{$form_name}_{$req['name']}", UOJLocale::get('problems::source code').':'.$req['file_name'], $languages);
...
EOD;
		...
		$form->handle = function() use($name, $requirement, $zip_file_name_gen, $handle) {
			...
			if ($requirement[0]['type'] == 'zip'){
				//copy all things	
				for($i = 0; $i < $up_zip_file->numFiles; $i++){
					$stat = $up_zip_file->statIndex($i);
					if ($stat === false) {
						$zip_content[$stat['name']] = '';
					} else {
						$tot_size += $stat['size'];
						if ($stat['size'] > 20 * 1024 * 1024) {
							becomeMsgPage("文件 {$stat['name']} 实际大小过大。");
						}
						$ret = $up_zip_file->getFromName($stat['name']);
						if ($ret === false) {
							$zip_content[$stat['name']] = '';
						} else {
							$zip_content[$stat['name']] = $ret;
						}
					}
				}
			...
			if ($requirement[0]['type'] == 'zip'){
				foreach($zip_content as $key => $value){
					$zip_file->addFromString($key, $value);
				}
			}else{
				foreach ($requirement as $req) {

<?php
	...
	if (isSuperUser($myUser)) {
		...
		$config_form = new UOJForm('config');
		$config_form->addInput('extra_config', 'text', '其它配置', json_encode($contest['extra_config']),
		...
		$config_form->handle = function(&$vdata) {
			global $contest;
			$esc_extra_config = DB::escape($vdata['extra_config']);
			DB::update("update contests set extra_config = '$esc_extra_config' where id = {$contest['id']}");
		};
		$config_form->runAtServer();
	}

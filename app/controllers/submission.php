<?php
	...
	if($contest && hasContestPermission($myUser, $contest)) {
		$stylescore_form = new UOJForm('stylescore');
		$stylescore_form->addInput('stylescore', 'text', '人工评测得分', $submission['stylescore'],
			function ($x, &$vdata) {
				if (!validateUInt($x) || $x < 0) {
					return '不是合法的得分';
				}
				$vdata['stylescore'] = $x;
				return '';
		...
		$stylescore_form->addTextArea('stylecomment', '评价', $submission['stylecomment'],
			function ($x, &$vdata) {
				$vdata['stylecomment'] = $x;
				return '';
			},
			null
		);
		$stylescore_form->handle = function(&$vdata) {
			global $submission;
			DB::update("update submissions set stylescore = '{$vdata["stylescore"]}', stylecomment = '{$vdata["stylecomment"]}' where id = {$submission['id']}");
			DB::update("update contests_submissions set stylescore = '{$vdata["stylescore"]}', stylecomment = '{$vdata["stylecomment"]}' where submission_id = {$submission['id']}");
		};
		$stylescore_form->runAtServer();
	}
...
<?php if ($should_show_content): ?>
	...
	<?php if (isset($stylescore_form)): ?>
		<p><a href="/download.php?type=submission&id=<?= $submission['id'] ?>">下载提交压缩包</a></p>
		<?php $stylescore_form->printHTML() ?>
	<?php endif ?>

	<?php if ($submission['stylecomment'] || $submission['stylescore']): ?>
		<div class="panel panel-info">
			<div class="panel-heading">
				<h4 class="panel-title">评价</h4>
			</div>
			<div class="panel-body">
				<pre><code class="'.$sh_class.'"> <?= $submission['stylecomment'] ?> </code></pre>
			</div>
			<div class="panel-footer">人工评分：<?= $submission['stylescore'] ?></div>
		</div>
	<?php endif ?>

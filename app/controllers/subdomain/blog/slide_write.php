<?php
	...
	if ($blog && !$blog['is_draft']) {
		$blog_editor->blog_url = "/blog/".UOJContext::user()['username']."/blog/{$blog['id']}";
	} else {
		...
		if ($blog) {
			if ($blog['is_draft']) {
				if ($data['is_hidden']) {
					...
					$ret['blog_write_url'] = "/blog/".UOJContext::user()['username']."/slide/{$blog['id']}/write";
					$ret['blog_url'] = "/blog/".UOJContext::user()['username']."/blog/{$blog['id']}";
				}
			} else {
				updateBlog($blog['id'], $data);
			}

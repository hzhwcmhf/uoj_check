<?php
	...
	function queryContestData() {
		...
		if ($contest['cur_progress'] < CONTEST_FINISHED) {
			$result = mysql_query("select id, submit_time, submitter, problem_id, score, stylescore from submissions where contest_id = {$contest['id']} and score is not null order by id");
		} else {
			$result = mysql_query("select submission_id, date_add('{$contest['start_time_str']}', interval penalty second), submitter, problem_id, score, stylescore from contests_submissions where contest_id = {$contest['id']}");
		}
		while ($row = mysql_fetch_array($result, MYSQL_NUM)) {
			$row[0] = (int)$row[0];
	...
	function calcStandings($contest_data, &$score, &$standings, $update_contests_submissions = false) {
		...
		// score: username, problem_pos => score, penalty, id, stylescore
		$score = array();
		...
		// standings: rank => score, penalty, [username, user_rating], stylescore, virtual_rank
		$standings = array();
		foreach ($contest_data['people'] as $person) {
			if ($update_contests_submissions && $person[1] < 0) {
				$person[1] = queryUser($person[0])['rating'];
				DB::update("update contests_registrants set user_rating = {$person[1]} where contest_id = {$contest['id']} and username = '{$person[0]}'");
			}
			$cur = array(0, 0, $person, 0);
			for ($i = 0; $i < $n_problems; $i++) {
				if (isset($score[$person[0]][$i])) {
					$cur_row = $score[$person[0]][$i];
					$cur[0] += $cur_row[0] * (1-$contest['extra_config']['style_proportion'][$i]);
					$cur[1] += $cur_row[1];
					$cur[3] += $cur_row[3] * $contest['extra_config']['style_proportion'][$i];
					if ($update_contests_submissions) {
						if(!isset($cur_row[3])) {
							$cur_row[3] = 0;
						}
						DB::insert("insert into contests_submissions (contest_id, submitter, problem_id, submission_id, score, penalty, stylescore) values ({$contest['id']}, '{$person[0]}', {$contest_data['problems'][$i]}, {$cur_row[2]}, {$cur_row[0]}, {$cur_row[1]}, {$cur_row[3]})");
					}
				}
			}
			$standings[] = $cur;
		...
		usort($standings, function($lhs, $rhs) {
			if ($lhs[0] + $lhs[3] != $rhs[0] + $rhs[3]) {
				return ($rhs[0] + $rhs[3] > $lhs[0] + $lhs[3]) ? 1:-1;
			} else if ($lhs[1] != $rhs[1]) {
				return $lhs[1] - $rhs[1];
			} else {
		...
		$is_same_rank = function($lhs, $rhs) {
			return $lhs[0] + $lhs[3] == $rhs[0] + $rhs[3] && $lhs[1] == $rhs[1];
		};
	...
	if (isSuperUser($myUser)) {
		...
		if ($contest['cur_progress'] >= CONTEST_TESTING) {
			...
			$publish_result_form->handle = function() {
				# TODO: refuse to update if the contest before didn't close
				$contest_before = queryContest($_GET['id']-1);
				genMoreContestInfo($contest_before);
				if ($contest_before['cur_progress'] < CONTEST_FINISHED && $_GET['id']!=1) {
					becomeMsgPage("上一场比赛还没有结束，不能公布成绩");
				}
				// time config
				set_time_limit(0);
...
EOD;
	...
	function echoStandings() {
		...
		echo 'style_proportion=', json_encode($contest['extra_config']['style_proportion']), ';';

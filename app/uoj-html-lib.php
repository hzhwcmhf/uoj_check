...
function echoSubmissionContent($submission, $requirement) {
	...
	foreach ($requirement as $req) {
		if ($req['type'] == "zip") {
			echo '<p><a href="/download.php?type=submission&id='.$submission['id'].'">下载提交压缩包</a></p>';
			break;
		}
		else if ($req['type'] == "source code") {
			$file_content = $zip_file->getFromName("{$req['file_name']}");
